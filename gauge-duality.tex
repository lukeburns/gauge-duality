...
\begin{document}
  ...
  \paragraph{Work In Progress} This paper is a work in progress, and is being openly developed on Github at \url{https://github.com/lukeburns/gauge-duality}. Contributions are warmly welcomed, whether by means of opening an issue or pull request.
  ...
  \begin{equation}
  M(x) = \int G(x, x') F(x') |d^{m}x'| - \oint G(x, x') n^{-1} d^{m-1}x' M(x'),
\end{equation}
